    ...
    if (pattern != nullptr) {
      if (regexec(pattern, stream->request.getUrl().data(), 1, matches, 0) == 0) {
        if (stream->request.c_opt.no_store == false) {
          service->http_cache->handleResponse(stream->response, stream->request);
        } else {
          service->http_cache->stats.cache_not_stored++;
        }
      }
    }
  } else {
    if (service->cache_enabled && service->http_cache->getCacheObject(stream->request) != nullptr &&
        !stream->request.c_opt.no_store && stream->response.c_opt.cacheable) {
      service->http_cache->addData(
          stream->response, std::string_view(stream->backend_connection.buffer, stream->backend_connection.buffer_size),
...
int CacheManager::handleRequest(HttpStream *stream, Service *service, ListenerConfig &listener_config_) {
  if (service->cache_enabled) {
    ...
    std::chrono::duration<long> time_span =
        std::chrono::duration_cast<std::chrono::duration<long>>(current_time - stream->prev_time);
    ...
    if (service->cache_enabled && service->http_cache->canBeServedFromCache(stream->request) != nullptr) {
      DEBUG_COUNTER_HIT(cache_stats__::cache_match);
      stream->response.reset_parser();
      if (service->http_cache->getResponseFromCache(stream->request, stream->response,
    ...
    if (it != http::http_info::headers_names.end()) {
      ...
      switch (header_name) {
        case http::HTTP_HEADER_NAME::CONTENT_LENGTH: {
          response.content_length = static_cast<size_t>(std::atoi(response.headers[i].value));
          continue;
        }
        case http::HTTP_HEADER_NAME::CACHE_CONTROL: {
          ...
          for (unsigned long l = 0; l < cache_directives.size(); l++) {
            ...
            if (cache_directives[l].back() == ',')
              cache_directives[l] = cache_directives[l].substr(0, cache_directives[l].length() - 1);
            ...
            if (http::http_info::cache_control_values.count(directive.data()) > 0) {
              switch (http::http_info::cache_control_values.at(directive.data())) {
                ...
                case http::CACHE_CONTROL::S_MAXAGE:
                  if (directive_value.size() != 0) response.c_opt.max_age = stoi(directive_value.data());
                  break;
            ...
            if (http::http_info::cache_control_values.count(directive.data()) > 0) {
              switch (http::http_info::cache_control_values.at(directive.data())) {
                ...
                case http::CACHE_CONTROL::MAX_STALE:
                  if (directive_value.size() != 0) request.c_opt.max_stale = stoi(directive_value.data());
                  break;
                case http::CACHE_CONTROL::MIN_FRESH:
                  if (directive_value.size() != 0) request.c_opt.min_fresh = stoi(directive_value.data());
                  break;
                case http::CACHE_CONTROL::NO_CACHE:
                ...
                case http::CACHE_CONTROL::ONLY_IF_CACHED:
                  ...
                  break;
                default:
                  zcutils_log_print(LOG_ERR, "Malformed cache-control, found response directive %s in the request",
                                directive.data());
                  break;
              }
            } else {
              zcutils_log_print(LOG_ERR, "Unrecognized directive %s in the request", directive.data());
            }
          }
          break;
        }

...
void CacheManager::handleResponse(HttpStream *stream, Service *service)
{
	...
	if (!stream->response.hasPendingData()) {
		...
		if (pattern != nullptr) {
			if (regexec(pattern, stream->request.getUrl().data(), 1,
				    matches, 0) == 0) {
				if (stream->request.c_opt.no_store == false) {
					service->http_cache->handleResponse(
						stream->response,
						stream->request);
				} else {
				...
				ListenerConfig &listener_config_)
{
	if (service->cache_enabled) {
		...
		if (service->cache_enabled &&
		    service->http_cache->canBeServedFromCache(
			    stream->request) != nullptr) {
			DEBUG_COUNTER_HIT(cache_stats__::cache_match);
			stream->response.reset_parser();
			if (service->http_cache->getResponseFromCache(
				    stream->request, stream->response,
				    stream->backend_connection.str_buffer) ==
			    0) {
				http_manager::validateResponse(
					*stream, listener_config_);
	...
	for (auto i = 0; i != static_cast<int>(response.num_headers); i++) {
		...
		if (it != http::http_info::headers_names.end()) {
			...
			case http::HTTP_HEADER_NAME::CACHE_CONTROL: {
				...
				for (unsigned long l = 0;
					...
					string_view directive(
						cache_directives[l].substr(
							0, cache_directives[l]
								   .find('=')));
					string_view directive_value(
						cache_directives[l].substr(
							cache_directives[l].find(
								'=') +
								1,
							cache_directives[l]
									.size() -
					...
					if (http::http_info::cache_control_values
						    .count(directive.data()) >
					    0) {
						switch (http::http_info::cache_control_values
								.at(directive
									    .data())) {
						case http::CACHE_CONTROL::MAX_AGE:
							if (directive_value.size() !=
								    0 &&
			...
			case http::HTTP_HEADER_NAME::EXPIRES:
				response.expires = time_helper::strToTime(
					std::string(header_value));
				break;
			case http::HTTP_HEADER_NAME::DATE:
				response.date = time_helper::strToTime(
					std::string(header_value));
				break;
			case http::HTTP_HEADER_NAME::LAST_MODIFIED:
				response.last_mod = time_helper::strToTime(
					std::string(header_value));
				break;
			case http::HTTP_HEADER_NAME::TRANSFER_ENCODING:
				...
				case 'd': // deflate
					response.transfer_encoding_type = http::
				...
				for (unsigned long l = 0;
					...
					if (http::http_info::cache_control_values
						...
						case http::CACHE_CONTROL::
							...
							break;
						case http::CACHE_CONTROL::NO_CACHE:
							request.c_opt.no_cache =
								true;
							break;
						case http::CACHE_CONTROL::NO_STORE:
							request.c_opt.no_store =
								true;
							break;
						case http::CACHE_CONTROL::
							NO_TRANSFORM:
							request.c_opt.transform =
								false;
							break;
						case http::CACHE_CONTROL::
							ONLY_IF_CACHED:
						...
						default:
							zcu_log_print(
								LOG_ERR,
								"Malformed cache-control, found response directive %s in the request",
								directive.data());
							break;
						}
					} else {

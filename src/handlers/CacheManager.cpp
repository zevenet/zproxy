...
void CacheManager::handleResponse(HttpStream *stream, Service *service )
{
    ...
    if ( !stream->response.hasPendingData()){
        ...
        if (service->cache_enabled && service->http_cache->getCacheObject(stream->request) != nullptr &&
          ...
          service->http_cache->addData(stream->response, std::string_view (stream->backend_connection.buffer,
...
int CacheManager::handleRequest(HttpStream * stream, Service *service, ListenerConfig &listener_config_ ) {
    if ( service->cache_enabled ){
        ...
        if ( service->cache_enabled && service->http_cache->canBeServedFromCache(stream->request) != nullptr) {
            ...
            if ( service->http_cache->getResponseFromCache(stream->request, stream->response, stream->backend_connection.str_buffer) == 0){

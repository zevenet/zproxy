...
ssize_t http_manager::getLastChunkSize(const char *data, size_t data_size, size_t &data_offset,
  ...
  if (chunk_size > 0) {
    ...
    if (data_size > (static_cast<size_t>(offset) + http::CRLF_LEN)) {
      ...
      return getLastChunkSize(data_ptr, data_size - static_cast<size_t>(offset), data_offset, chunk_size_bytes_left,
                              content_length);
    } else {
...
void http_manager::setBackendCookie(Service *service, HttpStream *stream) {
  if (!service->becookie.empty()) {
    ...
    if (!service->becdomain.empty()) set_cookie_header += "; Domain=" + service->becdomain;
    if (!service->becpath.empty()) set_cookie_header += "; Path=" + service->becpath;
...
validation::REQUEST_RESULT http_manager::validateRequest(HttpRequest &request,
  ...
  if (listener_config_.has_pat && regexec(&listener_config_.url_pat, request.path, 0, nullptr, 0)) {
    return validation::REQUEST_RESULT::BAD_URL;
  }

  // Check reqeuest size .
  if (UNLIKELY(listener_config_.max_req > 0 && request.headers_length > static_cast<size_t>(listener_config_.max_req) &&
    ...
    if (it != http::http_info::headers_names.end()) {
      ...
      switch (header_name) {
        ...
        case http::HTTP_HEADER_NAME::CONTENT_LENGTH: {
          request.content_length = static_cast<size_t>(std::atoi(request.headers[i].value));
          if ((request.content_length - request.message_length) > 0)
            request.message_bytes_left = request.content_length - request.message_length;
          continue;
        }
...
validation::REQUEST_RESULT http_manager::validateResponse(HttpStream &stream, const ListenerConfig &listener_config_) {
  ...
  for (size_t i = 0; i != response.num_headers; i++) {
    ...
    if (it != http::http_info::headers_names.end()) {
      ...
      switch (header_name) {
        case http::HTTP_HEADER_NAME::CONTENT_LENGTH: {
          stream.response.content_length = static_cast<size_t>(std::atoi(header_value.data()));
          if ((stream.response.content_length - stream.response.message_length) > 0)
            stream.response.message_bytes_left = stream.response.content_length - stream.response.message_length;
          continue;
        }
        ...
        case http::HTTP_HEADER_NAME::LOCATION: {
          // Rewrite location
          std::string location_header_value(response.headers[i].value, response.headers[i].value_len);
          if (listener_config_.rewr_loc == 0) continue;
          regmatch_t matches[4];
          ...
          std::string proto(location_header_value.data() + matches[1].rm_so,
                            static_cast<size_t>(matches[1].rm_eo - matches[1].rm_so));
          std::string host(location_header_value.data() + matches[2].rm_so,
                           static_cast<size_t>(matches[2].rm_eo - matches[2].rm_so));

          //        if (location_header_value[matches[3].rm_so] == '/') {
          //          matches[3].rm_so++;
          //        }
          std::string path(location_header_value.data() + matches[3].rm_so,
          ...
          if (host_ip == listener_config_.address || host_ip == stream.backend_connection.getBackend()->address) {
...
void http_manager::replyError(http::Code code, const std::string &code_string, const std::string &str,
                              Connection &target, SSLConnectionManager *ssl_manager) {
  size_t result;
  char caddr[200];
...
void http_manager::replyRedirect(HttpStream &stream, SSLConnectionManager *ssl_manager) {
  ...
  auto response_ = http::getRedirectResponse(
      static_cast<http::Code>(stream.backend_connection.getBackend()->backend_config.be_type), new_url);
...
void http_manager::replyRedirect(int code, const std::string &url, Connection &target,
                                 SSLConnectionManager *ssl_manager) {
  auto response_ = http::getRedirectResponse(static_cast<http::Code>(code), url);
  if (!target.ssl_connected) {
    target.write(response_.c_str(), response_.length());
  } else if (ssl_manager != nullptr) {
    size_t written = 0;

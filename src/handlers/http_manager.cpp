...
ssize_t http_manager::getChunkSize(const std::string &data, size_t data_size,
  ...
  if (pos != std::string::npos && pos < data_size) {
    ...
    if (*error != 0) {
      Debug::logmsg(LOG_NOTICE, "strtol() failed: Data size: %d  Buffer: %.*s",
                    data_size, 10, data.data());
      return -1;
    } else {
#if PRINT_DEBUG_CHUNKED
      Debug::logmsg(LOG_DEBUG, "CHUNK found size %s => %d ", hex.data(),
                    chunk_length);
#endif
...
void http_manager::applyCompression(Service *service, HttpStream *stream) {
  ...
  if ((stream->response.chunked_status == CHUNKED_STATUS::CHUNKED_DISABLED) &&
    ...
    stream->request.getHeaderValue(http::HTTP_HEADER_NAME::ACCEPT_ENCODING,
                                   compression_value);

    /* Check if we accept any of the compression algorithms. */
    size_t initial_pos;
    initial_pos =
        compression_value.find(service->service_config.compression_algorithm);
    if (initial_pos != std::string::npos) {
      ...
      switch (compression_type) {
        case http::TRANSFER_ENCODING_TYPE::GZIP: {
          ...
          if (!zlib::compress_message_gzip(message_no_compressed,
                                           message_compressed_gzip))
            Debug::logmsg(LOG_ERR, "Error while compressing.");
          strncpy(stream->response.message, message_compressed_gzip.c_str(),
                  stream->response.message_length);
          break;
        }
        case http::TRANSFER_ENCODING_TYPE::DEFLATE: {
          std::string message_compressed_deflate;
          if (!zlib::compress_message_deflate(message_no_compressed,
                                              message_compressed_deflate))
            Debug::logmsg(LOG_ERR, "Error while compressing.");
          strncpy(stream->response.message, message_compressed_deflate.c_str(),
                  stream->response.message_length);
          break;
        }
        default:
...
validation::REQUEST_RESULT http_manager::validateRequest(
    HttpRequest &request, const ListenerConfig &listener_config_) {  // FIXME
...
#if PRINT_DEBUG_CHUNKED
                  Debug::logmsg(LOG_REMOVE, ">>>> Chunk size %d left %d ",

...
//#if PRINT_DEBUG_CHUNKED
	...
	Debug::logmsg(LOG_REMOVE,
				  "[%s] buffer size: %6lu chunk left: %8d => Chunk size: %8d Data offset: %6lu Content_length: %8d  next chunk left %8d",
				  status,
...
//#endif
	if (chunk_size < 0) {
//	  const char *new_chunk_buff = stream.backend_connection.buffer + stream.response.chunk_size_left - 5;
	  /* here we have a tricky situation, we have received the last pendind part of last chunk, bute not enough data to process next chunk size */
	  //	  stream.response.chunk_buffer_size_offset = stream.backend_connection.buffer_size - stream.response.chunk_size_left;
	  return -1;
	  ...
	  stream.response.chunk_size_left = new_chunk_left < 0 ? 0 :new_chunk_left;
	}
...
ssize_t http_manager::getChunkSize(const std::string &data, size_t data_size, int &chunk_size_line_len) {
  ...
  //  Debug::logmsg(LOG_NOTICE, "Chunk not found, need more data: Buff size: %d Buff %.*s ",data_size, 5, data.data());
  return -1;
}

ssize_t http_manager::getLastChunkSize(const char *data,
									   size_t data_size,
									   size_t &data_offset,
  ...
  if (chunk_size > 0) {
	...
	if (data_size > (static_cast<size_t>(offset) + http::CRLF_LEN)) {
	  ...
	  return getLastChunkSize(data_ptr, data_size - static_cast<size_t>(offset), data_offset, chunk_size_bytes_left, content_length);
	} else {
...
validation::REQUEST_RESULT http_manager::validateRequest(HttpRequest &request, const ListenerConfig & listener_config_) {  //FIXME
  ...
  if (UNLIKELY(listener_config_.max_req > 0 &&
			   request.headers_length > static_cast<size_t>(listener_config_.max_req) &&
	...
	if (it != http::http_info::headers_names.end()) {
	  ...
	  switch (header_name) {
		...
		case http::HTTP_HEADER_NAME::LOCATION: {
			...
			std::string proto(location_header_value.data() + matches[1].rm_so, static_cast<size_t>(matches[1].rm_eo - matches[1].rm_so));
			std::string host(location_header_value.data() + matches[2].rm_so,static_cast<size_t>(matches[2].rm_eo - matches[2].rm_so));

			//        if (location_header_value[matches[3].rm_so] == '/') {
			//          matches[3].rm_so++;
			//        }
			std::string path(location_header_value.data() + matches[3].rm_so, static_cast<size_t>(matches[3].rm_eo - matches[3].rm_so));

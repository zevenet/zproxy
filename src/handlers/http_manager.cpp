			   ...
			   int &chunk_size_line_len)
{
	...
	if (pos != std::string::npos && pos < data_size) {
		chunk_size_line_len =
			static_cast < int >(pos) + http::CRLF_LEN;
	...
	for (size_t i = 0; i != request.num_headers; i++) {
		...
		if (it != http::http_info::headers_names.end()) {
						...
						if (header_value[1] == 'h') {	// no content-length
							...
							if (request.
								...
								if (chunk_size
									...
									request.chunked_status = CHUNKED_STATUS::CHUNKED_LAST_CHUNK;
								}
								else {
							 ...
							 'o') {
							request.transfer_encoding_type = TRANSFER_ENCODING_TYPE::COMPRESS;
						}
						break;
					}
				case 'd':	// deflate
			...
			case http::HTTP_HEADER_NAME::X_FORWARDED_FOR:{
					request.x_forwarded_for_string =
						header_value;
					request.headers[i].header_off = true;
					break;
				}

...
void httpsHeaders(HttpStream *stream, int clnt_check)
{
	...
	std::unique_ptr<X509, decltype(&::X509_free)> x509(
		SSL_get_peer_certificate(stream->client_connection.ssl),
		::X509_free);
	...
	if (clnt_check > 0 && x509 != nullptr) {
		...
		std::unique_ptr<BIO, decltype(&::BIO_free)> bb(
		...
		X509_NAME_print_ex(bb.get(), X509_get_issuer_name(x509.get()),
				   8, XN_FLAG_ONELINE & ~ASN1_STRFLGS_ESC_MSB);
		ssl::get_line(bb.get(), buf, ZCU_DEF_BUFFER_SIZE, &line_len);
		stream->request.addHeader(http::HTTP_HEADER_NAME::X_SSL_ISSUER,
					  buf, true);

		ASN1_TIME_print(bb.get(), X509_get_notBefore(x509.get()));
		ssl::get_line(bb.get(), buf, ZCU_DEF_BUFFER_SIZE, &line_len);
		stream->request.addHeader(
			http::HTTP_HEADER_NAME::X_SSL_NOTBEFORE, buf, true);

		ASN1_TIME_print(bb.get(), X509_get0_notAfter(x509.get()));
		ssl::get_line(bb.get(), buf, ZCU_DEF_BUFFER_SIZE, &line_len);
		stream->request.addHeader(
			http::HTTP_HEADER_NAME::X_SSL_NOTAFTER, buf, true);
		...
		header_value.erase(std::remove_if(header_value.begin(),
				   ...
				   header_value.end());
		stream->request.addHeader(
			http::HTTP_HEADER_NAME::X_SSL_CERTIFICATE, header_value,
			true);
	}
}

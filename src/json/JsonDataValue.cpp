...
void json::JsonDataValue::setValue(const json::JsonDataValue &value) {
  ...
  case JSON_VALUE_TYPE::JSON_T_NULL:setNullValue();
    break;
  case JSON_VALUE_TYPE::JSON_T_STRING:setValue(value.string_value);
    break;
  case JSON_VALUE_TYPE::JSON_T_BOOL:setValue(value.bool_value);
    break;
  case JSON_VALUE_TYPE::JSON_T_NUMBER:setValue(value.number_value);
    break;
  case JSON_VALUE_TYPE::JSON_T_OBJECT:setValue(*value.object_value);
    break;
  case JSON_VALUE_TYPE::JSON_T_ARRAY:setValue(*value.array_value);
    break;
  }
}
...
std::string json::JsonDataValue::stringify(bool prettyfy, int tabs) {
  switch (json_type) {
  case JSON_VALUE_TYPE::JSON_T_NULL:break;
  case JSON_VALUE_TYPE::JSON_T_STRING:return "\"" + string_value + "\"";
  case JSON_VALUE_TYPE::JSON_T_BOOL:return bool_value ? "true" : "false";
  case JSON_VALUE_TYPE::JSON_T_NUMBER:return std::to_string(number_value);
  case JSON_VALUE_TYPE::JSON_T_DOUBLE:return std::to_string(double_value);
  case JSON_VALUE_TYPE::JSON_T_OBJECT:return object_value->stringify(prettyfy, tabs + 1);
  case JSON_VALUE_TYPE::JSON_T_ARRAY:return array_value->stringify(prettyfy, tabs + 1);
  }

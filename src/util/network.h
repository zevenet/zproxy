...
{
      public:
	inline static char *getPeerAddress(int socket_fd, char *buf,
					   size_t bufsiz, bool include_port =
					   false) {
		int result;
	 ...
	 */
	inline static int getHost(const char *name, addrinfo * res,
		...
		if ((ret_val =
			...
			*res = *ap;
			if ((res->ai_addr =
			     static_cast <
			     sockaddr * >(std::malloc(ap->ai_addrlen))) ==
			    nullptr) {
				freeaddrinfo(chain);
				return EAI_MEMORY;
			}
			std::memcpy(res->ai_addr, ap->ai_addr,
				    ap->ai_addrlen);
			freeaddrinfo(chain);
		}
		return ret_val;
	...
	inline static std::unique_ptr < addrinfo,
		decltype(&::freeaddrinfo) >
		getAddress(const std::string & address, int port = 0) {
		addrinfo hints
		{
		};
		addrinfo *result
	...
	inline static char *getlocalAddress(int socket_fd, char *buf,
					    size_t bufsiz, bool include_port =
					    false) {
		int result;
		sockaddr_in adr_inet
		...
		case AF_INET:
			src = static_cast <
				...
				*>(&
				   (reinterpret_cast <
				    sockaddr_in *
				    >(addr->ai_addr))->sin_addr.s_addr);
			port = ntohs((reinterpret_cast <
				      sockaddr_in *
				      >(addr->ai_addr))->sin_port);
			if (inet_ntop
			    (AF_INET, src, buf,
	...
	inline static bool equalSockAddr(sockaddr * addr1, sockaddr * addr2,
		...
		if (addr1->sa_family == AF_UNIX) {
			auto a1_un =
				reinterpret_cast < sockaddr_un * >(addr1);
			auto a2_un =
				reinterpret_cast < sockaddr_un * >(addr2);
			...
			auto a1_in =
				reinterpret_cast < sockaddr_in * >(addr1);
			auto a2_in =
				reinterpret_cast < sockaddr_in * >(addr2);
			if (ntohl(a1_in->sin_addr.s_addr) !=
			    ntohl(a2_in->sin_addr.s_addr))
				return false;
			if (compare_port
			    && ntohs(a1_in->sin_port) !=
			    ntohs(a2_in->sin_port))
				return false;
		}
		else if (addr1->sa_family == AF_INET6) {
			auto a1_in6 =
				reinterpret_cast < sockaddr_in6 * >(addr1);
			auto a2_in6 =
				reinterpret_cast < sockaddr_in6 * >(addr2);
			...
			if (r != 0)
				return r;
			if (compare_port
			    && ntohs(a1_in6->sin6_port) !=
			    ntohs(a2_in6->sin6_port))
				return false;
	...
	inline static int setSocketSendBufferSize(int socket_fd,
	...
	inline static int setSocketReceiveBufferSize(int socket_fd,
						     unsigned int new_size)
	{
		unsigned int m = sizeof(new_size);

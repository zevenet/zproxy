...
class Environment {
 ...
 public:
  ...
  static bool setFileUserName(const std::string &user_name, const std::string &file_name) {
  ...
  static bool setFileGroupName(const std::string &group_name, const std::string &file_name) {
    if (!group_name.empty()) {
      struct group *gr;
      if ((gr = ::getgrnam(group_name.c_str())) == NULL) {
        Debug::logmsg(LOG_ERR, "no such group %s - aborted", group_name.c_str());
        return false;
      }
  ...
  static bool setFileUserMode(long user_mode, const std::string &file_name) {
    if (::chmod(file_name.c_str(), user_mode)) {
      Debug::logmsg(LOG_ERR, "chmod error on control socket - aborted (%s)", strerror(errno));
      return false;
    }
    return true;
  }
  ...
  static bool createPidFile(const std::string& pid_file_name, int pid = -1){
    ...
    if(pid_file_hl != NULL) {
      ...
      logmsg(LOG_NOTICE, "Create \"%s\": %s", pid_file_name.c_str(), strerror(errno));
    return  false;
  }

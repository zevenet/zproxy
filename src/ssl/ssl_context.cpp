...
SSLContext::init(const std::string & cert_file, const std::string & key_file)
{
	...
	SSL_CTX_set_options(ssl_ctx.get(), SSL_OP_ALL | SSL_OP_NO_SSLv2 | SSL_OP_NO_SSLv3);	// set ssl option
...
SSLContext::init(std::shared_ptr < BackendConfig > backend_config_)
{
	if (backend_config_->ctx != nullptr) {
		...
		if (method == nullptr)
			return false;
		this->ssl_ctx =
			std::shared_ptr < SSL_CTX > (SSL_CTX_new(method),
						     &::SSL_CTX_free);
		if (ssl_ctx == nullptr)
			return false;
		SSL_CTX_set_verify(this->ssl_ctx.get(), SSL_VERIFY_NONE,
				   nullptr);
		SSL_CTX_set_mode(this->ssl_ctx.get(),
				 SSL_MODE_RELEASE_BUFFERS);
		SSL_CTX_set_options(this->ssl_ctx.get(), SSL_OP_ALL);
#ifdef SSL_OP_NO_COMPRESSION
		SSL_CTX_set_options(this->ssl_ctx.get(),
...
SSLContext::initOpenssl()
{
	std::call_once(flag,[]() {
		       ...
		       OpenSSL_add_all_algorithms(); return true;}
	);
	return true;
}
...
SSLContext::SNIServerName(SSL * ssl, int dummy, POUND_CTX * ctx)
{
	...
	if ((server_name =
	     SSL_get_servername(ssl, TLSEXT_NAMETYPE_host_name)) == nullptr)

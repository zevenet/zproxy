 ...
 * Return 0 on success
 */
	static int get_line(BIO * const in, char *const buf,
			    const int bufsize, int *out_line_size)
	{
		char tmp;
		int i, n_read;
		...
		*out_line_size = 0;
		for (n_read = 0;;)
			switch (BIO_gets
				(in, buf + n_read, bufsize - n_read - 1)) {
			case -2:
	...
	static std::unique_ptr <
		char >ossGetSslSessionInfo(const SSL_SESSION * ses)
	{
		if (ses == nullptr)
			return nullptr;
		BIO *bio = BIO_new(BIO_s_mem());

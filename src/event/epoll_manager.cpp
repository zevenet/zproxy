...
/** Handles the read events. */
	...
	int EpollManager::loopOnce(int time_out)
	{
		int fd, i, ev_count = 0;
		ev_count =
			epoll_wait(epoll_fd, events, MAX_EPOLL_EVENT,
				   time_out);
		Time::updateTime();
#if USE_TIMER_FD_TIMEOUT==0
		...
		for (i = 0; i < ev_count; ++i) {
			fd = static_cast <
				int >(events[i].data.u64 >> CHAR_BIT);
			auto event_group =
				static_cast < EVENT_GROUP >
	...
	bool EpollManager::stopAccept(int listener_fd)
	{
		...
		for (auto it = accept_fd_set.begin();
		     it != accept_fd_set.end();) {
			if ((*it) == listener_fd) {
...
#if USE_TIMER_FD_TIMEOUT==0
	void EpollManager::setTimeOut(int fd, TIMEOUT_TYPE type,
				      int timeout_sec)
	{
		if (timeout_sec > 0) {

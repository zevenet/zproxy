...
void
http_parser::HttpData::reset_parser()
{
	method = nullptr;
	method_len = 0;
	path = nullptr;
...
void
http_parser::HttpData::setBuffer(char *ext_buffer, size_t ext_buffer_size)
{
	buffer = ext_buffer;
...
void
http_parser::HttpData::prepareToSend()
{
	iov_size = 0;
...
extra_headers) {		// header must be always  used as reference,
		...
		iov[iov_size++] =
			{const_cast < char *>(header.c_str()),
			 header.length()};
#if DEBUG_HTTP_HEADERS
		zcutils_log_print(LOG_DEBUG, "%.*s", header.length() - 2,
				  header.c_str());
#endif
	}
...
#if DEBUG_HTTP_HEADERS
		zcutils_log_print(LOG_DEBUG, "[%d bytes Content]",
				  message_length);
#endif
	}
}
...
void
http_parser::HttpData::addHeader(http::HTTP_HEADER_NAME header_name,
				 const std::string & header_value,
				 bool permanent)
{
	std::string newh;
	newh.reserve(http::http_info::headers_names_strings.at(header_name).
		     size() + http::CRLF_LEN + header_value.size() +
		     http::CRLF_LEN);
...
void
http_parser::HttpData::addHeader(const std::string & header_value,
				 bool permanent)
{
	std::string newh;
...
void
http_parser::HttpData::removeHeader(http::HTTP_HEADER_NAME header_name)
{
	auto
		header_to_remove =
	...
	extra_headers.
		erase(std::
		      remove_if(extra_headers.begin(), extra_headers.end(),
				[header_to_remove] (const std::
						    string & header) {
...
char *
http_parser::HttpData::getBuffer() const
{
	return buffer;
}

bool
http_parser::HttpData::getHeaderSent() const
{
	return headers_sent;
}

void
http_parser::HttpData::setHeaderSent(bool value)
{
	headers_sent = value;
}

bool
http_parser::HttpData::getHeaderValue(http::HTTP_HEADER_NAME header_name,
				      std::string & out_key)
{
	for (size_t i = 0; i != num_headers; ++i) {
		...
		if (http_info::headers_names.find(header) !=
			...
			auto
				header_name_ =
				http_info::headers_names.at(header);
...
bool
http_parser::HttpData::getHeaderValue(const std::string & header_name,
				      std::string & out_key)
{
	for (size_t i = 0; i != num_headers; ++i) {
...
http_parser::PARSE_RESULT http_parser::HttpData::parseRequest(const std::
							      string & data,
							      size_t
							      *used_bytes,
							      bool reset)
...
http_parser::PARSE_RESULT http_parser::HttpData::
parseRequest(const char *data, const size_t data_size, size_t *used_bytes,
	     [[maybe_unused]]
	     bool reset)
{
	//  if (LIKELY(reset))
	...
	const char **
		method_ = const_cast < const char **>(&method);
	const char **
		path_ = const_cast < const char **>(&path);
	auto
		pret =
		phr_parse_request(data, data_size, method_, &method_len,
				  path_,
	...
	if (pret > 0) {
		*used_bytes = static_cast < size_t >(pret);
		...
		http_version =
			minor_version ==
			1 ? HTTP_VERSION::HTTP_1_1 : HTTP_VERSION::HTTP_1_0;
		message = &buffer[pret];
...
http_parser::PARSE_RESULT http_parser::HttpData::parseResponse(const std::
							       string & data,
							       size_t
							       *used_bytes,
							       [[maybe_unused]]
...
http_parser::PARSE_RESULT http_parser::HttpData::
parseResponse(const char *data, const size_t data_size, size_t *used_bytes,
	      [[maybe_unused]]
	      bool reset)
{
	//  if (LIKELY(reset))
	...
	auto
		pret =
		phr_parse_response(data, data_size, &minor_version,
...
void
http_parser::HttpData::printResponse()
{
	zcutils_log_print(LOG_DEBUG, "HTTP 1.%d %d %s", minor_version,
			  http_status_code,
...
void
http_parser::HttpData::printRequest()
{
...
bool
http_parser::HttpData::hasPendingData()
{
	return headers_sent	/*&&
				   (message_bytes_left > 0 ||

...
enum class Code {
  ...
  SwitchingProtocols = 101,  //!< Indicates that the server understands and is
                             //!< willing to comply with the client's request,
                             //!< via the Upgrade header field, for a change in
                             //!< the application protocol being used on this
                             //!< connection.
  ...
  PartialContent = 206,  //!< Indicates that the server is successfully
                         //!< fulfilling a range request for the target resource
                         //!< by transferring one or more parts of the selected
                         //!< representation that correspond to the satisfiable
                         //!< ranges found in the requests's Range header field.
  MultiStatus = 207,  //!< Provides status for multiple independent operations.
  IMUsed = 226,  //!< The server has fulfilled a GET request for the resource,
                 //!< and the response is a representation of the result of one
                 //!< or more instance-manipulations applied to the current
                 //!< instance.
  ...
  MovedPermanently = 301,  //!< Indicates that the target resource has been
                           //!< assigned a new permanent URI and any future
                           //!< references to this resource ought to use one of
                           //!< the enclosed URIs.
  ...
  NotModified = 304,  //!< Indicates that a conditional GET request has been
                      //!< received and would have resulted in a 200 (OK)
                      //!< response if it were not for the fact that the
                      //!< condition has evaluated to false.
  UseProxy = 305,     //!< \deprecated
  TemporaryRedirect = 307,  //!< Indicates that the target resource resides
                            //!< temporarily under a different URI and the user
                            //!< agent MUST NOT change the request method if it
  ...
  NotAcceptable = 406,  //!< Indicates that the target resource does not have a
                        //!< current representation that would be acceptable to
                        //!< the user agent, according to the proactive
  ...
  RequestTimeout = 408,  //!< Indicates that the server did not receive a
                         //!< complete request message within the time that it
  ...
  PreconditionFailed = 412,  //!< Indicates that one or more preconditions given
                             //!< in the request header fields evaluated to
  ...
  UnsupportedMediaType = 415,  //!< Indicates that the origin server is refusing
                               //!< to service the request because the payload
                               //!< is in a format not supported by the target
  ...
  FailedDependency = 424,  //!< Means that the method could not be performed on
                           //!< the resource because the requested action
                           //!< depended on another action and that action
                           //!< failed.
  UpgradeRequired = 426,   //!< Indicates that the server refuses to perform the
                           //!< request using the current protocol but might be
                           //!< willing to do so after the client upgrades to a
                           //!< different protocol.
  PreconditionRequired = 428,  //!< Indicates that the origin server requires
                               //!< the request to be conditional.
  TooManyRequests = 429,       //!< Indicates that the user has sent too many
   ...
   */
  InternalServerError = 500,  //!< Indicates that the server encountered an
  ...
  BadGateway = 502,  //!< Indicates that the server, while acting as a gateway
                     //!< or proxy, received an invalid response from an inbound
                     //!< server it accessed while attempting to fulfill the
                     //!< request.
  ServiceUnavailable = 503,  //!< Indicates that the server is currently unable
                             //!< to handle the request due to a temporary
                             //!< overload or scheduled maintenance, which will
                             //!< likely be alleviated after some delay.
  GatewayTimeout = 504,      //!< Indicates that the server, while acting as a
                             //!< gateway or proxy, did not receive a timely
                             //!< response from an upstream server it needed to
                             //!< access in order to complete the request.
  HTTPVersionNotSupported = 505,  //!< Indicates that the server does not
                                  //!< support, or refuses to support, the
                                  //!< protocol version that was used in the
                                  //!< request message.
  ...
  InsufficientStorage = 507,  //!< Means the method could not be performed on
                              //!< the resource because the server is unable to
                              //!< store the representation needed to
...
inline std::string getHttpResponse(Code status_code,
  ...
  std::string code_error = status_code_string.empty()
                               ...
                               : status_code_string;
  std::string body = content_message.empty()
                         ? "<html>\n"
                           "<head><title>" +
...
inline std::string getRedirectResponse(Code code, std::string redirect_url) {
  /*HTTP/1.0 302 Found
   ...
   Content-Type: text/html
   Content-Length: 162
   <html><head><title>Redirect</title></head><body><h1>Redirect</h1><p>You
   should go to <a
   href="http://www.google.com/">http://www.google.com/</a></p></body></html>
   */
  ...
  //      + " </title > </head >\n"
  //        "<body bgcolor=\"white\">\n"
  //        "<center><h1>" + std::to_string(static_cast<int>(code)) + " " +
  //        code_error +
  //      "</h1></center>\n"
  //      "<hr><center>zhttp /0.1 </center>\n"
  //      "</body>\n"
  ...
  std::string body =
      "<html><head><title>Redirect</title></head><body><h1>Redirect</h1><p>You "
      "should go to <a href=" +
      redirect_url + ">" + redirect_url + "</a></p></body></html>";

...
namespace ctl
{
	...
	class ControlManager:public EpollManager, public CtlNotify < CtlTask,
		std::string > {
		...
		bool init(Config & configuration,
			  CTL_INTERFACE_MODE listener_mode =
			  CTL_INTERFACE_MODE::CTL_UNIX);
		void start();
		void stop();
		void sendCtlCommand(CTL_COMMAND command,
		...
		bool setListenerTarget(CtlTask & task,
				       std::istringstream & ss);
		bool setServiceTarget(CtlTask & task,
				      std::istringstream & ss);
		bool setBackendTarget(CtlTask & task,
				      std::istringstream & ss);
	};

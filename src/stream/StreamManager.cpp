...
void StreamManager::onRequestEvent(int fd) {
  ...
  do {
    ...
    switch (parse_result) {
      case http_parser::SUCCESS: {
        ...
        if (service == nullptr) {
          ...
          if (UNLIKELY(Network::getPeerAddress(fd, caddr, 50) == nullptr)) {
            ...
            Debug::logmsg(
                LOG_WARNING, "(%lx) e%d %s from %s", std::this_thread::get_id(),
      ...
      case http_parser::FAILED:
        Debug::Log("Parser FAILED", LOG_DEBUG);
        break;
      case http_parser::INCOMPLETE:
        Debug::Log("Parser INCOMPLETE", LOG_DEBUG);
        break;
      case http_parser::TOOLONG:
    ...
    if ((stream->client_connection.buffer_size - parsed) > 0) {
      ...
      Debug::Log("Parsed data size: " + std::to_string(parsed), LOG_DEBUG);
           ...
           parse_result ==
               http_parser::SUCCESS);  // TODO:: Add support for http pipeline
  ...
  if (stream->timer_fd.isTriggered()) {
    ...
    if (UNLIKELY(Network::getPeerAddress(
      ...
      Debug::logmsg(LOG_NOTICE, "(%lx) e%d %s %s from %s",
                    ...
                    validation::request_result_reason.at(
                        validation::REQUEST_RESULT::BACKEND_TIMEOUT),
                    stream->client_connection.buffer, caddr);
    }
    stream->replyError(
...
validation::REQUEST_RESULT StreamManager::validateRequest(
    HttpRequest& request) {  // TODO:: why use of e501

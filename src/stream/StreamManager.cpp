...
//    StreamWatcher watcher(*stream);
  ...
  case http_parser::PARSE_RESULT::SUCCESS: {
    auto valid = http_manager::validateRequest(stream->request, listener_config_);
    ...
    if (bck == nullptr) {
      ...
      IO::IO_OP op_state = IO::IO_OP::OP_ERROR;
                Debug::logmsg(LOG_DEBUG, "[%s] %.*s [%s (%d) -> %s (%d)]", service->name.c_str(),
...
void StreamManager::onResponseEvent(int fd) {
  ...
  if (UNLIKELY(stream->client_connection.isCancelled())) { //check if client is still active
    clearStream(stream);
    return;
  }
...
//    StreamWatcher watcher(*stream);
...
#if ENABLE_ZERO_COPY
        if (stream->response.message_bytes_left > 0 && !stream->backend_connection.getBackend()->isHttps() && !this->is_https_listener
  ...
  Debug::logmsg(LOG_REMOVE, "IO RESULT: %s", IO::getResultString(result).data());
    switch (result) {
        case IO::IO_RESULT::SSL_HANDSHAKE_ERROR:
        case IO::IO_RESULT::SSL_NEED_HANDSHAKE: {
    ...
    case IO::IO_RESULT::DONE_TRY_AGAIN: {
      if (stream->backend_connection.buffer_size==0) {
        if (stream->response.message_bytes_left>0 || stream->upgrade.pinned_connection)
  ...
  if (stream->upgrade.pinned_connection || stream->response.message_bytes_left>0
    ...
    if (stream->response.transfer_encoding_header) {
      stream->response.message_bytes_left = stream->backend_connection.buffer_size;
      stream->response.transfer_encoding_header = stream->backend_connection.buffer_size==0;
//            http_manager::getChunkSize(
//                    std::string(stream->backend_connection.buffer, stream->backend_connection.buffer_size));
    }
  ...
  if (ret!=http_parser::PARSE_RESULT::SUCCESS) {
    ...
    if (http_manager::validateResponse(*stream,listener_config_) != validation::REQUEST_RESULT::OK) {
      Debug::logmsg(LOG_NOTICE,
...
//  StreamWatcher watcher(*stream);
   ...
   * content without applying any kind of modification. */
    ...
    if (this->is_https_listener) {
     ...
     if(stream->backend_connection.buffer_size > 0)
        result = stream->backend_connection.writeTo(stream->client_connection.getFileDescriptor(), written);
#if ENABLE_ZERO_COPY

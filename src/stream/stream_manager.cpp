			   ...
			   EVENT_GROUP event_group)
{
	...
	default:
		zcutils_log_print(LOG_ERR, "%s():%d: unexpected  event type",
...
StreamManager::onRequestEvent(int fd)
{
	...
	ScopeExit logStream
	{
		[stream, &extra_log]
		{
			HttpStream::dumpDebugData(stream, "OnRequest",
						  extra_log.data());
		}
	};
#endif
	...
	case http_parser::PARSE_RESULT::TOOLONG:
		zcutils_log_print(LOG_DEBUG, "%s():%d: Parser TOOLONG",
				  __FUNCTION__, __LINE__);
		[[fallthrough]];
	...
	case http_parser::PARSE_RESULT::INCOMPLETE:
		zcutils_log_print(LOG_DEBUG, "%s():%d: Parser INCOMPLETE",
				  __FUNCTION__, __LINE__);
		return;
	}
...
StreamManager::onResponseEvent(int fd)
{
	...
	ScopeExit logStream
	{
		...
		{
			//check not null
			HttpStream::dumpDebugData(stream, "OnResponse",
						  extra_log.data());
		}
	};
#endif
...
#endif
	if (stream == nullptr) {
		zcutils_log_print(LOG_DEBUG, "%s():%d: stream null pointer",
				  __FUNCTION__, __LINE__);
		deleteFd(fd);
		::close(fd);
		return;
...
StreamManager::onRequestTimeoutEvent(int fd)
{
	...
	//~ StreamDataLogger logger(stream, listener_config_);
	zcutils_log_print(LOG_WARNING,
			  "(%lx) client request timeout after %d seconds",
			  pthread_self(),
...
StreamManager::onServerWriteEvent(HttpStream * stream)
{
	...
	ScopeExit logStream
	{
		[stream, &extra_log]
		{
			HttpStream::dumpDebugData(stream, "onServerW",
						  extra_log.data());
	}};
#endif
...
#if EXTENDED_DEBUG_LOG
			HttpStream::dumpDebugData(stream,
						  "ClientW-ReadPending",
						  "WROTE REQ PENDING ");
#endif
			//stream->client_connection.enableReadEvent();
	...
	if (stream->hasStatus(STREAM_STATUS::CL_READ_PENDING)) {
#if EXTENDED_DEBUG_LOG
		HttpStream::dumpDebugData(stream, "ClientW-ReadPending",
					  "WROTE REQ PENDING ");
#endif
...
StreamManager::onClientWriteEvent(HttpStream * stream)
{
	...
	ScopeExit logStream
	{
		[stream, &extra_log]
		{
			HttpStream::dumpDebugData(stream, "onClientW",
						  extra_log.data());
		}
	};
#endif
	...
	if (stream->hasOption(STREAM_OPTION::PINNED_CONNECTION)
		...
		default:
			...
			HttpStream::dumpDebugData(stream, "onServerW-ERROR",
						  error.data());
			clearStream(stream);
			return;
		}

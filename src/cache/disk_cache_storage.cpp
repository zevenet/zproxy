 ...
 * DISKst::STORAGE
 */
st::STORAGE_STATUS DiskCacheStorage::initCacheStorage(
  ...
  if (mkdir(m_point.data(), 0777) == -1) {
    ...
    if (errno != EEXIST) return st::STORAGE_STATUS::MKDIR_ERROR;
  }
...
st::STORAGE_STATUS DiskCacheStorage::initServiceStorage(
  ...
  if (mkdir((path).data(), 0777) == -1) {
    if (errno == EEXIST) return st::STORAGE_STATUS::MPOINT_ALREADY_EXISTS;
    if (errno != EEXIST) return st::STORAGE_STATUS::MKDIR_ERROR;
  }
...
st::STORAGE_STATUS DiskCacheStorage::getFromStorage(const std::string &rel_path,
...
st::STORAGE_STATUS DiskCacheStorage::putInStorage(const std::string &rel_path,
...
st::STORAGE_STATUS DiskCacheStorage::stopCacheStorage() {
  const std::filesystem::path path_m_point =
      std::filesystem::u8path(mount_path);
...
st::STORAGE_STATUS DiskCacheStorage::deleteInStorage(const std::string &path) {
  ...
  if (isInStorage(full_path)) {
    if (std::remove(full_path.data())) return st::STORAGE_STATUS::GENERIC_ERROR;

...
class BackendConfig:Counter < BackendConfig > {
	...
	int max_request
	{
	0};			/*Maximum request number allowed per connection */
	 ~BackendConfig()
	{
	}
};
...
struct ListenerConfig:Counter < ListenerConfig > {
	...
	int port;
	  std::shared_ptr < POUND_CTX > ctx
	{
	nullptr};		/* CTX for SSL connections */
	...
	MATCHER *head_off
	{
	nullptr};		/* headers to remove */
	MATCHER *response_head_off
	{
	nullptr};		/* headers to remove  from response */
	std::string ssl_config_file;	/* OpenSSL config file path */
	int rewr_loc
	{
	0};			/* rewrite location response */
	int rewr_dest
	{
	0};			/* rewrite destination header */
	int rewr_host
	{
	0};			/* rewrite host header */
	std::string ssl_config_section;	/* OpenSSL config section */
	int disabled
	{
	0};			/* true if the listener is disabled */
	...
	nullptr};
	std::shared_ptr < ListenerConfig > next {
	nullptr};
	~ListenerConfig() {

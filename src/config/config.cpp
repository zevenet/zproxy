...
Config::parse_file()
{
	...
	nullptr};
	std::shared_ptr < ListenerConfig > lstn {
	nullptr};
	int i;
	...
	while (conf_fgets(lin, ZCU_DEF_BUFFER_SIZE)) {
		...
		if (!regexec(&regex_set::User, lin, 4, matches, 0)) {
			...
			user = std::string(lin + matches[1].rm_so,
					   static_cast <
			...
			lin[matches[1].rm_eo] = '\0';
			group = std::string(lin + matches[1].rm_so,
					    static_cast <
					    size_t >(matches[1].rm_eo -
						     matches[1].rm_so));
		}
			...
			lin[matches[1].rm_eo] = '\0';
			name = std::string(lin + matches[1].rm_so,
					   static_cast <
					   size_t >(matches[1].rm_eo -
						    matches[1].rm_so));
		}
		...
		else if (!regexec(&regex_set::ThreadModel, lin, 4, matches, 0)) {	// ignore
			// threadpool = ((lin[matches[1].rm_so] | 0x20) == 'p'); /* 'pool' */ //
		}
		else if (!regexec
			 (&regex_set::LogFacility, lin, 4, matches, 0)) {
			lin[matches[1].rm_eo] = '\0';
			...
			be_connto = atoi(lin + matches[1].rm_so);
			 ...
			 (&regex_set::Ignore100continue, lin, 4, matches,
			  0)) {
			ignore_100 = atoi(lin + matches[1].rm_so);
		}
		else if (!regexec(&regex_set::IgnoreCase, lin, 4, matches, 0)) {
		...
		else if (!regexec(&regex_set::SSLEngine, lin, 4, matches, 0)) {
			lin[matches[1].rm_eo] = '\0';
		...
		else if (!regexec(&regex_set::ControlIP, lin, 4, matches, 0)) {
			lin[matches[1].rm_eo] = '\0';
			ctrl_ip =
				std::string(lin + matches[1].rm_so,
					    static_cast <
			 ...
			 (&regex_set::ControlPort, lin, 4, matches, 0)) {
			lin[matches[1].rm_eo] = '\0';
			ctrl_port = atoi(lin + matches[1].rm_so);
		}
		else if (!regexec
			 (&regex_set::ControlUser, lin, 4, matches, 0)) {
			lin[matches[1].rm_eo] = '\0';
			ctrl_user =
				std::string(lin + matches[1].rm_so,
					    static_cast <
			 ...
			 (&regex_set::ControlGroup, lin, 4, matches, 0)) {
			lin[matches[1].rm_eo] = '\0';
			ctrl_group =
				std::string(lin + matches[1].rm_so,
					    static_cast <
			 ...
			 (&regex_set::ControlMode, lin, 4, matches, 0)) {
			lin[matches[1].rm_eo] = '\0';
			ctrl_mode =
			...
			if (listeners == nullptr)
			 ...
			 (&regex_set::ListenHTTPS, lin, 4, matches, 0)) {
			if (listeners == nullptr)
			...
			if (services == nullptr)
				...
				for (svc = services; svc->next;
				     svc = svc->next);
				svc->next = parseService(nullptr);
			}
		}
		else if (!regexec
			 (&regex_set::ServiceName, lin, 4, matches, 0)) {
			lin[matches[1].rm_eo] = '\0';
			if (services == nullptr)
				services =
		...
		else if (!regexec(&regex_set::Anonymise, lin, 4, matches, 0)) {
			anonymise = 1;
#ifdef CACHE_ENABLED
		}
		else if (!regexec
			 (&regex_set::CacheThreshold, lin, 2, matches, 0)) {
			...
			cache_s = size;
			 ...
			 (&regex_set::CacheRamPath, lin, 2, matches, 0)) {
			cache_ram_path =
				std::string(lin + matches[1].rm_so,
					    matches[1].rm_eo -
					    matches[1].rm_so);
			 ...
			 (&regex_set::CacheDiskPath, lin, 2, matches, 0)) {
			cache_disk_path =
				std::string(lin + matches[1].rm_so,
					    matches[1].rm_eo -
					    matches[1].rm_so);
	...
	while (conf_fgets(lin, ZCU_DEF_BUFFER_SIZE)) {
		...
		if (!regexec(&regex_set::Address, lin, 4, matches, 0)) {
			...
			if (regcomp(&res->url_pat, lin + matches[1].rm_so,
				    REG_NEWLINE | REG_EXTENDED | (ignore_case
								  ? REG_ICASE
								  : 0)))
		...
		else if (!regexec(&regex_set::MaxRequest, lin, 4, matches, 0)) {
			res->max_req = atoll(lin + matches[1].rm_so);
		}
		else if (!regexec(&regex_set::HeadRemove, lin, 4, matches, 0)) {
			if (res->head_off) {
		...
		else if (!regexec(&regex_set::AddHeader, lin, 4, matches, 0)) {
			lin[matches[1].rm_eo] = '\0';
			if (res->add_head.empty()) {
			 ...
			 (&regex_set::RewriteLocation, lin, 4, matches, 0)) {
			res->rewr_loc = std::atoi(lin + matches[1].rm_so);
		}
		else if (!regexec
			 (&regex_set::RemoveResponseHeader, lin, 4, matches,
			  0)) {
			...
			if (regcomp(&m->pat, lin + matches[1].rm_so,
			 ...
			 (&regex_set::AddResponseHeader, lin, 4, matches,
			  0)) {
			if (res->response_add_head.empty()) {
			 ...
			 (&regex_set::RewriteDestination, lin, 4, matches,
			  0)) {
			res->rewr_dest = atoi(lin + matches[1].rm_so);
		}
		else if (!regexec
			 (&regex_set::RewriteHost, lin, 4, matches, 0)) {
			...
			if (res->services == nullptr)
			 ...
			 (&regex_set::ReplaceHeader, lin, 5, matches, 0)) {
			lin[matches[1].rm_eo] = '\0';
	...
	while (conf_fgets(lin, ZCU_DEF_BUFFER_SIZE)) {
		...
		if (!regexec(&regex_set::Address, lin, 4, matches, 0)) {
			...
			res->errnossl = file2str(lin + matches[1].rm_so);
			 ...
			 (&regex_set::NoSslRedirect, lin, 4, matches, 0)) {
			res->nossl_redir = 302;
		...
		else if (!regexec(&regex_set::ForwardSNI, lin, 4, matches, 0)) {
		...
		else if (!regexec(&regex_set::ClientCert, lin, 4, matches, 0)) {
			has_other = 1;
			if (res->ctx == nullptr)
			...
			if (res->add_head.empty()) {
			 ...
			 (&regex_set::DisableProto, lin, 4, matches, 0)) {
			lin[matches[1].rm_eo] = '\0';
			...
			else if (strcasecmp(lin + matches[1].rm_so, "TLSv1_1")
				 == 0)
				ssl_op_enable |=
					SSL_OP_NO_SSLv2 | SSL_OP_NO_SSLv3 |
			...
			else if (strcasecmp(lin + matches[1].rm_so, "TLSv1_2")
				 == 0)
				ssl_op_enable |=
					SSL_OP_NO_SSLv2 | SSL_OP_NO_SSLv3 |
					SSL_OP_NO_TLSv1 | SSL_OP_NO_TLSv1_1 |
			...
			else if (strcasecmp(lin + matches[1].rm_so, "TLSv1_3")
				 == 0)
			...
			if ((res->ecdh_curve_nid =
			 ...
			 (&regex_set::SSLAllowClientRenegotiation, lin, 4,
			  matches, 0)) {
			...
			if (res->allow_client_reneg == 2) {
			 ...
			 (&regex_set::SSLHonorCipherOrder, lin, 4, matches,
			  0)) {
			...
			for (pc = res->ctx; pc; pc = pc->next)
				if (SSL_CTX_load_verify_locations
				    (pc->ctx.get(), lin + matches[1].rm_so,
				     nullptr) != 1)
		...
		else if (!regexec(&regex_set::NoHTTPS11, lin, 4, matches, 0)) {
			...
			if (regcomp(&m->pat, lin + matches[1].rm_so,
			 ...
			 (&regex_set::SSLUncleanShutdown, lin, 4, matches,
			  0)) {
	...
	*count_ = 0;
	...
	for (i = 0; i < local_count; i++) {
		result[i] =
			...
			*>(strndup
			   (reinterpret_cast <
			    const char *>(temp[i]),::strlen(reinterpret_cast <
							    const char
							    *>(temp[i])) +
			    1));
		if (result[i] == nullptr)
		  ...
		  char *filename)
{
	...
	if (SSL_CTX_use_PrivateKey_file
	    (pc->ctx.get(), filename, SSL_FILETYPE_PEM) != 1)
...
// conf_err("ListenHTTPS: could not get certificate CN");
	...
	if (SSL_CTX_use_certificate_chain_file(res->ctx->ctx, filename) != 1)
		conf_err("SSL_CTX_use_certificate_chain_file failed - aborted");
	if (SSL_CTX_use_PrivateKey_file
	    (res->ctx->ctx, filename, SSL_FILETYPE_PEM) != 1)
		conf_err("SSL_CTX_use_PrivateKey_file failed - aborted");
...
std::shared_ptr < ServiceConfig > Config::parseService(const char *svc_name)
{
	...
	while (conf_fgets(lin, ZCU_DEF_BUFFER_SIZE)) {
		...
		if (!regexec(&regex_set::URL, lin, 4, matches, 0)) {
			...
			lin[matches[1].rm_eo] = '\0';
			if (regcomp(&m->pat, lin + matches[1].rm_so,
				    REG_NEWLINE | REG_EXTENDED | (ign_case ?
								  REG_ICASE :
								  0)))
			...
			free(ptr);
			 ...
			 (&regex_set::HeadRequire, lin, 4, matches, 0)) {
			if (res->req_head) {
			...
			if (regcomp(&m->pat, lin + matches[1].rm_so,
			 ...
			 (&regex_set::StrictTransportSecurity, lin, 4,
			  matches, 0)) {
			...
			if (regexec
			    (&regex_set::LOCATION, be->url.data(), 4, matches,
			     0))
				conf_err("Redirect bad URL - aborted");
			if ((matches[3].rm_eo - matches[3].rm_so) == 1)	/* the path is a single '/', so remove it */
				be->url.pop_back();
		...
		else if (!regexec(&regex_set::Emergency, lin, 4, matches, 0)) {
			res->emergency = parseBackend(svc_name, 1);
		}
		else if (!regexec
			 (&regex_set::BackendCookie, lin, 5, matches, 0)) {
			lin[matches[1].rm_eo] = '\0';
			lin[matches[2].rm_eo] = '\0';
			lin[matches[3].rm_eo] = '\0';
			lin[matches[4].rm_eo] = '\0';
			...
			if (matches[2].rm_so != matches[2].rm_eo &&
			    (res->becdomain =
			     strdup(lin + matches[2].rm_so)) == nullptr)
				conf_err("out of memory");
			if (matches[3].rm_so != matches[3].rm_eo &&
			    (res->becpath =
			     strdup(lin + matches[3].rm_so)) == nullptr)
				conf_err("out of memory");
			...
			res->dynscale = atoi(lin + matches[1].rm_so) == 1;
			 ...
			 (&regex_set::PinnedConnection, lin, 4, matches, 0)) {
			res->pinned_connection =
				std::atoi(lin + matches[1].rm_so) == 1;
		}
		else if (!regexec
			 (&regex_set::RoutingPolicy, lin, 4, matches, 0)) {
			lin[matches[1].rm_eo] = '\0';
			...
			if (cp == "ROUND_ROBIN")
			 ...
			 (&regex_set::CompressionAlgorithm, lin, 4, matches,
			  0)) {
			lin[matches[1].rm_eo] = '\0';
			std::string cp = lin + matches[1].rm_so;
						       ...
						       const int is_emergency)
{
	...
	while (conf_fgets(lin, ZCU_DEF_BUFFER_SIZE)) {
		...
		if (!regexec(&regex_set::Address, lin, 4, matches, 0)) {
			...
			if ((addr.ai_family == AF_INET
			     || addr.ai_family == AF_INET6) && !has_port)
...
Config::conf_fgets(char *buf, const int max)
{
	...
	for (;;) {
		...
		if (!regexec(&regex_set::Empty, buf, 4, matches, 0) || !regexec(&regex_set::Comment, buf, 4, matches, 0))	/* comment or empty line */
			continue;
		if (!regexec(&regex_set::Include, buf, 4, matches, 0)) {
			...
			if ((f_in[cur_fin] =
			     fopen(&buf[matches[1].rm_so], "rt")) == nullptr)
...
Config::include_dir(const char *conf_path)
{
	...
	while ((de = readdir(dp)) != nullptr) {
		...
		if ((strlen(de->d_name) >= 5 &&
		    ...
		    || (strlen(de->d_name) >= 6
			&& !strncmp(de->d_name + strlen(de->d_name) - 5,
				    ".conf", 5))) {

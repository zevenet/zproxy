  ...
  if (task.command == ctl::CTL_COMMAND::GET) {
    switch (task.subject) {
      case ctl::CTL_SUBJECT::STATUS: {
        ...
        switch (this->status) {
          case BACKEND_STATUS::BACKEND_UP:
            status_.emplace(JSON_KEYS::STATUS, new JsonDataValue(JSON_KEYS::STATUS_UP));
            break;
          case BACKEND_STATUS::BACKEND_DOWN:
            status_.emplace(JSON_KEYS::STATUS, new JsonDataValue(JSON_KEYS::STATUS_DOWN));
            break;
          case BACKEND_STATUS::BACKEND_DISABLED:
            status_.emplace(JSON_KEYS::STATUS, new JsonDataValue(JSON_KEYS::STATUS_DISABLED));
            break;
          default:
            status_.emplace(JSON_KEYS::STATUS, new JsonDataValue(JSON_KEYS::UNKNOWN));
            break;
        }
      ...
      case ctl::CTL_SUBJECT::WEIGHT: {
        JsonObject weight_;
        weight_.emplace(JSON_KEYS::WEIGHT, new JsonDataValue(this->weight));
        return weight_.stringify();
      }
      default:
        auto backend_status = this->getBackendJson();
        if (backend_status != nullptr) return backend_status->stringify();
        return JSON_OP_RESULT::ERROR;
    }
  } else if (task.command == ctl::CTL_COMMAND::UPDATE) {
    auto status_object = JsonParser::parse(task.data);
    if (status_object == nullptr) return JSON_OP_RESULT::ERROR;
    switch (task.subject) {
...
std::unique_ptr<JsonObject> Backend::getBackendJson() {
  ...
  if (this->backend_type != BACKEND_TYPE::REDIRECT) {
    ...
    switch (this->status) {
      case BACKEND_STATUS::BACKEND_UP:
        root->emplace(JSON_KEYS::STATUS, std::unique_ptr<JsonDataValue>(new JsonDataValue(JSON_KEYS::STATUS_ACTIVE)));
        break;
      case BACKEND_STATUS::BACKEND_DOWN:
        root->emplace(JSON_KEYS::STATUS, std::unique_ptr<JsonDataValue>(new JsonDataValue(JSON_KEYS::STATUS_DOWN)));
        break;
      case BACKEND_STATUS::BACKEND_DISABLED:
        root->emplace(JSON_KEYS::STATUS, std::unique_ptr<JsonDataValue>(new JsonDataValue(JSON_KEYS::STATUS_DISABLED)));
        break;
      default:
    ...
    root->emplace(JSON_KEYS::PENDING_CONNS,
                  std::unique_ptr<JsonDataValue>(new JsonDataValue(this->pending_connections)));
...
void Backend::doMaintenance() {
  ...
  switch (res) {
    case IO::IO_OP::OP_SUCCESS: {
      ...
      Logger::logmsg(LOG_NOTICE, "BackEnd %s:%d resurrect in farm: '%s', service: '%s'", this->address.data(),
                    this->port, this->backend_config.f_name.data(), this->backend_config.srv_name.data());
      this->status = BACKEND_STATUS::BACKEND_UP;
      break;
    }
    default:

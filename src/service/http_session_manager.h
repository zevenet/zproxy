...
namespace sessions
{
	...
	class HttpSessionManager
	{
		...
		std::unordered_map < std::string, SessionInfo * >sessions_set;	// key can be anything, depending on the session type
	      public:
		HttpSessionType session_type;
		std::string sess_id;	/* id to construct the pattern */
		regex_t sess_start
		{
		};		/* pattern to identify the session data */
		regex_t sess_pat
		{
		};		/* pattern to match the session data */
		...
		SessionInfo *addSession(Connection & source,
					HttpRequest & request,
		...
		void deleteSession(Connection & source,
				   HttpRequest & request);
		// return the assigned backend or nullptr if no session is found or sesssion
		// has expired
		SessionInfo *getSession(Connection & source,
					HttpRequest & request,

...
bool sessions::HttpSessionManager::updateSession(
	Connection &source, HttpRequest &request,
	const std::string &new_session_id, Backend &backend_to_assign)
{
	...
	if (!session_id.empty()) {
		...
		session_data = session_data == nullptr ? new SessionInfo() :
							       session_data;
...
void HttpSessionManager::deleteSession(Connection &source, HttpRequest &request)
{
	std::lock_guard<std::recursive_mutex> locker(lock_mtx);
...
bool HttpSessionManager::deleteSession(const JsonObject &json_object)
{
	...
	if (json_object.count(JSON_KEYS::BACKEND_ID) > 0 &&
		...
		if (it != json_object.end() &&
		    json_object.at(JSON_KEYS::ID)->isValue()) {
			std::string key =
				dynamic_cast<JsonDataValue *>(it->second.get())
					->string_value;
...
std::string
HttpSessionManager::getCookieValue(std::string_view cookie_header_value,

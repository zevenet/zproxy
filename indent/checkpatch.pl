...
# hack to not check Linux SPDX License
my $checkspdxlicense = 0;
my $quiet = 0;
my $verbose = 0;
my %verbose_messages = ();
my %verbose_emitted = ();
...
# check for using SPDX license tag at beginning of files
		if ($realline == $checklicenseline && $checkspdxlicense) {
			if ($rawline =~ /^[ \+]\s*\#\!\s*\//) {
				$checklicenseline = 2;
			} elsif ($rawline =~ /^\+/) {
				my $comment = "";
					 ...
					 $op eq '%')
				{
					if ($check) {
						...
						if ($realfile =~ /\.cpp|\.h$/) {
							...
							# Ignore rvalue in function declaration "funct(int &val)"
 							if ($op eq '&' && $ctx =~ /WxV/) {
 								$ok2 = 1;
 							}
						}
...
# check for function definitions
		if ($perl_version_ok &&
			...
			$ok = 1 if ($brac_ident eq $func_ident);

...
bool zcu_soc_equal_sockaddr(sockaddr *addr1, sockaddr *addr2, bool compare_port)
{
...
bool zcu_soc_set_socket_non_blocking(int fd, bool blocking)
{
...
bool zcu_soc_set_solingeroption(int sock_fd, bool enable)
{
	struct linger l {
	};
...
/*
	 ...
	 */
int zcu_net_get_host(const char *name, addrinfo *res, int ai_family, int port)
{
...
zcu_net_get_address(const std::string &address, int port)
{
	addrinfo hints{};
...
void zcu_net_addr2str(char *const res, size_t res_len,
		      const struct addrinfo *addr, bool include_port)
{
...
bool zcu_net_equal_sockaddr(addrinfo *x, addrinfo *y, bool compare_port)
{

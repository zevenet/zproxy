...
inline static int zcu_soc_get_local_port(int socket_fd)
{
	...
	if (::getsockname(socket_fd, reinterpret_cast<sockaddr *>(&adr_inet),
			  &len_inet) != -1) {
...
inline static bool zcu_soc_equal_sockaddr(sockaddr *addr1, sockaddr *addr2,
					     ...
					     bool include_port = false)
{
	...
	socklen_t len_inet = sizeof adr_inet;
	result = ::getpeername(
		socket_fd, reinterpret_cast<sockaddr *>(&adr_inet), &len_inet);
	if (result == -1) {
		return nullptr;
	}
...
inline static int zcu_soc_get_peer_port(int socket_fd)
{
	...
	if (::getpeername(socket_fd, reinterpret_cast<sockaddr *>(&adr_inet),
			  &len_inet) != -1) {
		port = ntohs(adr_inet.sin_port);
		return port;
	}
...
inline static bool zcu_soc_set_solingeroption(int sock_fd, bool enable = false)
{
	struct linger l {
	};
...
inline static bool zcu_soc_set_somarkoption(int sock_fd, int nf_mark)
{
	...
	return nf_mark != 0 && setsockopt(sock_fd, SOL_SOCKET, SO_MARK,
					  &nf_mark, sizeof(nf_mark)) != -1;
}
...
inline static int zcu_soc_set_socketsendbuffersize(int socket_fd,
						   unsigned int new_size)
{
	unsigned int m = sizeof(new_size);
	return ::setsockopt(socket_fd, SOL_SOCKET, SO_SNDBUF, &new_size, m) !=
	       -1;
}

inline static int zcu_soc_set_socketreceivebuffersize(int socket_fd,
						      unsigned int new_size)
{
	unsigned int m = sizeof(new_size);
	return ::setsockopt(socket_fd, SOL_SOCKET, SO_RCVBUF, &new_size, m) !=
	       -1;
}
...
/*
	 ...
	 */
inline static int zcu_net_get_host(const char *name, addrinfo *res,
				   int ai_family = AF_UNSPEC, int port = 0)
{
	...
	if ((ret_val = getaddrinfo(
		     name, port > 0 ? std::to_string(port).data() : nullptr,
		     &hints, &chain)) == 0) {
...
inline static std::unique_ptr<addrinfo, decltype(&::freeaddrinfo)>
...
/*
	 ...
	 */
static void zcu_net_addr2str(char *const res, size_t res_len,
	...
	case AF_INET:
		src = static_cast<void *>(
			&(reinterpret_cast<sockaddr_in *>(addr->ai_addr))
				 ->sin_addr.s_addr);
		port = ntohs((reinterpret_cast<sockaddr_in *>(addr->ai_addr))
				     ->sin_port);
		if (inet_ntop(AF_INET, src, buf, ZCU_DEF_BUFFER_SIZE - 1) ==
		    nullptr)
	...
	case AF_INET6:
		src = static_cast<void *>(
			&(reinterpret_cast<sockaddr_in6 *>(addr->ai_addr))
				 ->sin6_addr.s6_addr);
		port = ntohs((reinterpret_cast<sockaddr_in6 *>(addr->ai_addr))
				     ->sin6_port);
		if (IN6_IS_ADDR_V4MAPPED(
			...
			if (inet_ntop(AF_INET, src, buf,
				      ZCU_DEF_BUFFER_SIZE - 1) == nullptr)
				strncpy(buf, "(UNKNOWN)",
					ZCU_DEF_BUFFER_SIZE - 1);
		} else {
			if (inet_ntop(AF_INET6, src, buf,
				      ZCU_DEF_BUFFER_SIZE - 1) == nullptr)
				strncpy(buf, "(UNKNOWN)",
					ZCU_DEF_BUFFER_SIZE - 1);
		}
		break;
	case AF_UNIX:
		strncpy(buf, reinterpret_cast<char *>(addr->ai_addr),
			ZCU_DEF_BUFFER_SIZE - 1);
		port = 0;
		break;
	default:
...
inline static bool zcu_net_equal_sockaddr(addrinfo *x, addrinfo *y,
					  bool compare_port = true)
{
	return zcu_soc_equal_sockaddr(x->ai_addr, y->ai_addr, compare_port);
}

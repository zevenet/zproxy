...
=begin nd
Function: runRBLStartByFarm

	Link a farm with all its RBL rules. If some rule is not been used by another farm, the rule is runned
	It is useful when a farm is started or add to rule
...
sub runRBLStartByFarm
{
	my $farmname = shift;

	# get all applied rules for this farm
	foreach my $rule ( @{ &getRBLFarmApplied( $farmname ) } )
	{
...
=begin nd
Function: runRBLStopByFarm

	Unlink a farm with all its RBL rules. If no more farm is using the rule, stop it.
...
sub runRBLStopByFarm
{
	...
	foreach my $rule ( @{ &getRBLFarmApplied( $farmname ) } )
	{
		# Remove the iptables rule to unlink with rbl rule
		&runRBLIptablesRule( $rule, $farmname, 'delete' );
...
=begin nd
Function: runRBLRestartByFarm

	Reload all RBL rules to a farm.
...
sub runRBLRestartByFarm
{
	my ( $farmname ) = @_;

	my $error = &runRBLStopByFarm( $farmname );

	if ( !$error )
	{
		$error = &runRBLStartByFarm( $farmname );
	}
...
sub runRBLStopByRule
{
	...
	if ( !$error )
	{
		...
		&zenlog( "RBL rule \"$rule\" has been stopped." );
	}
...
sub getRBLFarmApplied
{
	...
	foreach my $rule ( @{ &getRBLRuleList() })
	{
		if ( grep ( /^$farmname$/, @{ &getRBLObjectRuleParam( 'farms' ) } ) )
		{
			push @rules, $rule;
		}
	}

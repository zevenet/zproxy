...
print "This script modifies the configuration files from ZEN Load Balancer Enterprise Edition beyond v3.02...\n";
...
foreach $file(@l4farms){
	...
	my $line = @filelines[0];
	my @args = split("\;",$line);
	my $line = "@args[0]\;tcp\;@args[1]\;@args[2]\;@args[3]\;@args[4]\;@args[5]\;@args[6]\;@args[7]";
	@filelines[0] = $line;
	print "file migrated\n";
	untie $file;
	$oldfile = $file;
...
foreach $file(@l4farms){
	...
	my $line = @filelines[0];
	my @args = split("\;",$line);
	my $line = "@args[0]\;udp\;@args[1]\;@args[2]\;@args[3]\;@args[4]\;@args[5]\;@args[6]\;@args[7]";
	@filelines[0] = $line;
	print "file migrated\n";
	untie $file;
	$oldfile = $file;

		...
		-buttons  => [
			{
			   ...
			   -onpress  => sub {
				   my $ret =
					 &confirm_dialog( "Are you sure you want to reboot your ZEVENET server?" );
				   if ( $ret )
				   {
					   my @run = `(reboot &)`;
					   exit ( 0 );
			...
			},
			{
			   ...
			   -onpress  => sub {
				   my $ret =
					 &confirm_dialog( "Are you sure you want to shutdown your ZEVENET server?" );
				   if ( $ret )
				   {
					   my @run = `(poweroff &)`;
					   exit ( 0 );
	...
	my $service2 = $win3->add(
		...
		-buttons => [
			...
			},
			{
			   ...
			   -onpress  => sub {
				   my $ret =
					 &confirm_dialog( "Are you sure you want to START ZEVENET service?" );
				   if ( $ret )
				   {
			...
			},
			{
			   ...
			   -onpress  => sub {
				   my $ret =
					 &confirm_dialog( "Are you sure you want to RESTART ZEVENET service?" );
				   if ( $ret )
				   {
...
sub set_new_hostname
{
	...
	{
		my $ret = &confirm_dialog(
					   "Are you sure you want to change your ZEVENET hostname?" );
		if ( $ret )
		{

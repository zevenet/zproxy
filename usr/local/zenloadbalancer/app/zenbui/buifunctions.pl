...
sub get_system_mem(){
	my $line;
      	my ($mname, $mvalue, $mfname, $mfvalue, $mused, $mbname, $mbvalue, $mcname, $mcvalue, $swtname, $swtvalue, $swfname, $swfvalue, $swused);
	if (-f "/proc/meminfo"){
...
sub set_data_string(){
	...
	my $j = 0;
 	for $i ( 0 .. $#datain ) {
		$outputstring = $outputstring . "\t" . $datain[$i][0] . ": " . $datain[$i][1] . "\n";
	}
	return $outputstring;
}

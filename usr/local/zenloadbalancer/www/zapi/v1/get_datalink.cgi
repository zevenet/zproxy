...
#	-u zapi:<password> https://<zenlb_server:444/zapi/v1/zapi.cgi/farms/DATAFARM
#
...
foreach $l_servers(@run){
        ...
        if ($l_serv[1] ne "0.0.0.0"){
            push $out_b, { id => $l_serv[0], ip => $l_serv[1], interface => @l_serv[2], weight => @l_serv[3], priority => @l_serv[4] };
        }
}

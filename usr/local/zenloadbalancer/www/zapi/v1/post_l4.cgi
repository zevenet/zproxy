...
# L4XNAT
#
# curl --tlsv1 -k -X POST -H 'Content-Type: application/json' -H "ZAPI_KEY: MyIzgr8gcGEd04nIfThgZe0YjLjtxG1vAL0BAfST6csR9Hg5pAWcFOFV1LtaTBJYs" -u zapi:admin -d '{"ip":"1.2.1.2","port":"79","weight":"2","priority":"3"}' https://178.62.126.152:445/zapi/v1/zapi.cgi/farms/L4FARM/backends
#
#
#
...
#       "weight":"2"}' https://<zenlb_server>:444/zapi/v1/zapi.cgi/farms/L4FARM/backends
#
...
####### Create backend
	...
	$status = &setFarmServer($id,$json_obj->{ip},$json_obj->{port},"",$json_obj->{weight},$json_obj->{priority},"",$farmname);
                if ($status != -1){
                        ...
                        push $out_p, { id => $id, ip => $json_obj->{ip}, port => $json_obj->{port}+0, weight => $json_obj->{weight}+0, priority => $json_obj->{priority}+0 };

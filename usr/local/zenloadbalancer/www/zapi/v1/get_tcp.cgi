...
push $out_p, { algorithm => &getFarmAlgorithm($1), persistence => &getFarmPersistence($1), maxclients => $maxclients, tracking => $maxtimemem , timeout => $timeout, connmax => $conn_max, maxservers => $maxbackends, blacklist => $blacklist, fgenabled => $fguse, fgtimecheck => $fgttcheck, fgscript => $fgscript, fglog => $fglog, xforwardedfor => $xforw,  vip => $vip, vport => $vport };
#
#backends
...
foreach $l_servers(@run){
        ...
        if ($l_serv[2] != "0.0.0.0"){
                push $out_b, { id => $l_serv[0], ip => $l_serv[2], port => @l_serv[4], maxconns => @l_serv[8], weight => @l_serv[12], priority => @l_serv[14] };
        }
}

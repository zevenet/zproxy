...
#       "maxthreads":"259","certname":"zencert.pem"}' https://<zenlb_server>:444/zapi/v2/zapi.cgi/farms/FarmHTTP
#
...
if ( exists ( $json_obj->{ vip } ) && !exists ( $json_obj->{ vport } ) )
{
	...
	else
	{
		...
		{
			$error = "true";
			&logfile( "ZAPI error, trying to modify a http farm $farmname, invalid vip." );
		}
		else
		{
			$restart_flag = "true";
...
if ( exists ( $json_obj->{ vport } ) && !exists ( $json_obj->{ vip } ) )
{
	...
	{
		$error = "true";
		&logfile( "ZAPI error, trying to modify a http farm $farmname, invalid port." );
	}
	else
	{

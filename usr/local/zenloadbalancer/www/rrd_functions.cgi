...
sub genLoadGraph    #($type,$graph,$time)
{
	...
	if ( -e "$rrdap_dir$rrd_dir$db_load" )
	{
		...
		RRDs::graph(
					 ...
					 "STACK:load5#DC374A:last 5 minutes\\t",
...
sub genFarmGraph    #($type,$graph,$time)
{
	...
	if ( -e "$rrdap_dir$rrd_dir$db_farm" )
	{
		...
		RRDs::graph(
			...
			"LINE2:established#729e00:Established\\t",

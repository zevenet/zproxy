...
$command .= " --header \"Subject: " . &getData('PrefixSubject', $section) . " $subject\"";
$command .= " --body \"$body\"";
#not print
$command .= " 1>/dev/null";
...
sub getSubjectBody   # &getSubjectBody ( $msg ) 
{
	...
	my ($date, $host, $program, $ip, $port, $status, $farm, $service);
	my $auxBody;
	...
	# pound msg
	# example:
	# Jul 28 11:16:46 maqvir pound: (7f4dccf24700) BackEnd 192.168.0.172:80 dead (killed) in farm: 'test', service: 'srv1'
	if ( ( $program =~ /pound/ || $program =~ /farmguardian/ ) &&
		 $msg =~ /BackEnd (\d+\.\d+\.\d+\.\d+):(\d+) (\w+)(?: \(\w+\))? in farm: '(\w+)', service: '(\w+)'/ )
	{
		$ip = $1;
		$port = $2;
		$status = $3;
		...
		if ( $status =~ /dead/ || $status =~ /down/ )
			{ $status = "DOWN"; }
		elsif ( $status =~ /resurrect/ )
			{ $status = "UP"; }

		if ($farm && !$service)
			{ $auxBody = "(farm: '$farm')"; }
		if ($farm && $service)
			{ $auxBody = "(farm: '$farm', service: '$service')"; }
...
Port: $port
	...
	# Cluster msg
	# example:
	# Aug 16 11:47:58 maqvir ucarp[11541]: [WARNING] Switching to state: MASTER
	elsif ( $program =~ /ucarp/ &&

...
my $bkdir = "/opt/fg_bk";
mkdir $bkdir if ( !-d $bkdir );
my @conf;
my $obj;

# foreach fg, remove old fg and create a new one. Link the fg to the farm if it is enabled
foreach my $farm ( &getFarmNameList() )
{
	#~ $fg_conf =~ /([^_]+)_?(.+)?_guardian.conf/;
	...
	if ( $type =~ /(http|gslb)/ )
	{
		foreach my $srv ( &getFarmServices( $farm ) )
		{
			if ( $type =~ /http/ )
			{
				@conf = &getFarmGuardianConf( $farm, $srv );

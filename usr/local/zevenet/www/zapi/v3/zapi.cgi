...
&zenlog( "CGI POST DATA: " . $post_data, "debug", "ZAPI" ) if $post_data && &debug && $ENV{ CONTENT_TYPE } eq 'application/json';
&zenlog( "CGI PUT DATA: " . $put_data, "debug", "ZAPI" )   if $put_data && &debug && $ENV{ CONTENT_TYPE } eq 'application/json';
...
POST qr{^/session$} => sub {
	...
	if ( $session && !$session->param( 'is_logged_in' ) )
	{
		...
		{
			&zenlog( "Login failed for username: $username", "info", "ZAPI" );

...
{
	require Zevenet::API3::Certificate::Ciphers;
...
if ( $q->path_info =~ qr{/ipds/rbl} )
{
	...
	GET qr{^/ipds/rbl/domains$} => sub {
		&get_rbl_domains;
	};

	#  POST /ipds/rbl/domains
	POST qr{^/ipds/rbl/domains$} => sub {
		&add_rbl_domain( @_ );
	};

	#  PUT /ipds/rbl/domains/<domain>
	PUT qr{^/ipds/rbl/domains/($rbl_domain)$} => sub {
		&set_rbl_domain( @_ );
	};

	#  DELETE /ipds/rbl/domains/<domain>
	DELETE qr{^/ipds/rbl/domains/($rbl_domain)$} => sub {
		&del_rbl_domain( @_ );
	};
	...
	#  POST /ipds/rbl/<name>
	POST qr{^/ipds/rbl/($rbl_name)$} => sub {
		&copy_rbl_rule( @_ );
	};

	#  PUT /ipds/rbl/<name>
	PUT qr{^/ipds/rbl/($rbl_name)$} => sub {
		&set_rbl_rule( @_ );
	};

	#  DELETE /ipds/rbl/<name>
	DELETE qr{^/ipds/rbl/($rbl_name)$} => sub {
		&del_rbl_rule( @_ );
	};

	#  POST /ipds/rbl/<name>/domains
	POST qr{^/ipds/rbl/($rbl_name)/domains$} => sub {
		&add_domain_to_rbl( @_ );
	};

	#  DELETE /ipds/rbl/<name>/domains/<domain>
	DELETE qr{^/ipds/rbl/($rbl_name)/domains/($rbl_domain)$} => sub {
		&del_domain_from_rbl( @_ );
	};
	...
	DELETE qr{^/farms/($farm_re)/ipds/rbl/($rbl_name)$} => sub {
		&del_rbl_from_farm( @_ );
	};

	# POST /ipds/rbl/<name>/actions
	POST qr{^/ipds/rbl/($rbl_name)/actions$} => sub {
		&set_rbl_actions( @_ );
	};
}

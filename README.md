...
- **RewriteLocation** 0|1 [0|1]
	...
	Changes the Location and Content-Location headers in the responses to show the virtual host that was sent in the request.
	...
	The value 1 (by default) enables host rewrites.
...
- **RewriteUrl** "pattern" "replace" [last]
	...
	Checks a pattern in order to get strings from URL and replace them.
...
- **AddRequestHeader** "header: to add"
	...
	Overwrites the listener *AdddrequestHeader*. Add the defined header to the request passed to the back-end server. The header is added verbatim. Use multiple AddRequestHeader directives if you need to add more than one header.
...
- **Type** IP|BASIC|URL|PARM|COOKIE|HEADER|BACKENDCOOKIE
	...
	- **BACKENDCOOKIE**: Zproxy will inject a cookie in each response with the appropriate backend's key, so that even if the session table is flushed or sessions are disabled, the proper backend can be chosen. This allows for session databases to be offloaded to the client side via browser cookies.
